...
bool a_star_manhattan(int start, int end){
    vector<int> v;
    dis[start] = WW*heuristic_manhattan(start, end);
    G[start] = 0;
    v.push_back(start);
    state[start] = 1;
    ...
    while(!v.empty()){
        ...
        state[node] = 2;
        if( ascii_mode )
            cout << " Node : " << setw(3) << (char)(node+'A') << " f(n): " << setw(3) << dis[node] << " g(n): " << setw(3) << G[node] << " h(n): " << setw(3) <<WW*heuristic_manhattan(node, end) << endl;
        else
            cout << " Node : " << setw(3) << node+1 << " f(n): " << setw(3) << dis[node] << " g(n): " << setw(3) << G[node] << " h(n): " << setw(3) << WW*heuristic_manhattan(node, end) << endl;
        if( node == end )
            return true;
        for(int c: moveGen(node)){
            if( state[c] == 0 ){
                ...
                dis[c] = G[c] + WW*heuristic_manhattan(c, end);
                v.push_back(c);
...
bool a_star_euclidean(int start, int end){
    vector<int> v;
    dis[start] = WW*heuristic_eucledian(start, end);
    G[start] = 0;
    v.push_back(start);
    state[start] = 1;
    make_heap(v.begin(), v.end(), Comparator());
    while(!v.empty()){
        ...
        for(int c: moveGen(node)){
            if( state[c] == 0 ){
                ...
                parent[c] = node;
                G[c] = G[node] + edges[c][node];
                dis[c] = G[c] + WW*heuristic_eucledian(c, end);
                v.push_back(c);
                push_heap(v.begin(), v.end(), Comparator());
            }else if( state[c] == 1){
                if( G[c] > (G[node] + edges[c][node])){
